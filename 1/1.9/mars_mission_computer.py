...
def info_loop(computer):
    while True:
        computer.get_mission_computer_info()
        time.sleep(20)

def load_loop(computer):
    while True:
        computer.get_mission_computer_load()
        time.sleep(20)

runComputer = MissionComputer()
t_info = threading.Thread(target=info_loop, args=(runComputer,))
t_load = threading.Thread(target=load_loop, args=(runComputer,))
t_sensor = threading.Thread(target=runComputer.get_sensor_data)
t_info.start()
t_load.start()
t_sensor.start()
...
p_info = Process(target=info_loop, args=(runComputer1,))
p_load = Process(target=load_loop, args=(runComputer2,))
p_sensor = Process(target=runComputer3.get_sensor_data)
p_info.start()
p_load.start()
p_sensor.start()
